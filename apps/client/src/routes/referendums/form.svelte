<script lang="ts">
  ...
  import { toArrayValue, toDatetimeInput } from '@/lib/form';
  ...
  type Referendum = GetReferendumByIdQuery['referendum'];

  export let referendum: Referendum | undefined = undefined;

  const pageBaseTrans = 'domains.referendum.form';
  ...
  const { form, data, addField, unsetField, errors, setFields } = createForm<FormValues>({
    onSubmit(values) {
      ...
      return client.CreateOneReferendum(formatter.toCreateOneInput());
    },
    initialValues: {
      ...
      startDate: toDatetimeInput(referendum?.startDate),
      endDate: toDatetimeInput(referendum?.endDate),
      answers: toArrayValue(referendum?.answers),
      participantsExternalIds: toArrayValue(referendum?.participantsExternalIds),
      participantsRoles: toArrayValue(referendum?.participantsRoles),
      participantsKind: referendum?.participantsKind || ReferendumParticipantsKind.All,
      answerKind: referendum?.answerKind || ReferendumAnswerKind.YesNo
    },
  ...
  const title = referendum
    ? $_('pages.referendums:id:edit.title')
    : $_('pages.referendums:create.title');
...
<div class="space-y-4">
  <h3 class="capitalize-first text-xl">{title}</h3>
